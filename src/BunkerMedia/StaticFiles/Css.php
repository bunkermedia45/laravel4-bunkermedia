...
class Css extends File {
     ...
     * @param $css_file
     * @param null $media - условие использования файла
     * @param null $condition - condition including script, example [if IE 6]
     * @param bool $no_build - флаг исключения из сборки
     * @return $this
    ...
    function add($css_file, $media = null, $condition = null, $no_build = false) {
        ...
        if (mb_strpos($css_file, '/') === 0) {
            $css_file = \Config::get('staticfiles.host') . $css_file;
        }
    ...
    function getInline($as_html = true) {
        ...
        $css_inline = $this->prepare($css_inline);
        if (\Config::get('staticfiles.css.build', false)) {
